...
// EmptyShare returns a share with zero values. This instance
// ca be used to unmarshal a share's binary representation.
func EmptyShare(g group.Group) Share {
	return Share{
		ID:    g.NewScalar(),
		Value: g.NewScalar(),
	}
...
// MarshalBinary returns the binary representation of a share.
func (s Share) MarshalBinary() ([]byte, error) {
	id, err := s.ID.MarshalBinary()
	if err != nil {
		return nil, err
	}
	value, err := s.Value.MarshalBinary()
	if err != nil {
		return nil, err
	}
	return append(id, value...), nil
}

// UnmarshalBinary restores a share from its binary representation.
func (s Share) UnmarshalBinary(data []byte) error {
	if err := s.ID.UnmarshalBinary(data[:len(data)/2]); err != nil {
		return err
	}
	if err := s.Value.UnmarshalBinary(data[len(data)/2:]); err != nil {
		return err
	}
	return nil
}
...
// SecretSharing provides a (t,n) Shamir's secret sharing. It allows
// splitting a secret into n shares, such that the secret can be only
...
// Share creates `n` shares with an ID monotonically increasing from
// 1 to n.
...
// ShareWithID creates one share of the secret using `id` as identifier.
// Notice that shares with the same ID are considered equal. Panics, if
// the `id` provided is zero.
func (ss SecretSharing) ShareWithID(id group.Scalar) Share {
	if id.IsZero() {
		panic("vss: id cannot be zero")
	}
...
// CommitSecret creates a commitment to the secret for further verifying
// shares.
...
// Verify returns true if the share `s` was produced by sharing a secret
// with threshold `t` and commitment of the secret `c`.
...
// Recover returns a secret provided more than `t` different shares are
// given. Returns an error if the number of shares is not above the
// threshold `t`. Panics if some shares are duplicated, i.e., shares must
// have different IDs.
